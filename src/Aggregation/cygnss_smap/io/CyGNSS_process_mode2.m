function CyGNSSproduct_atResolution = CyGNSS_process_mode2(Resolution, cygnss_data, cygnss_vars,numcols, numrows)
...
for i=1:numel(cygnss_vars) % initialize the varibales in the structure
    ...
    if size(cygnss_data.(cygnss_vars{i}))==size(cygnss_r)% check for some variables with the wrong size like year or doy
        ...
        elseif string(var)=="REFLECTIVITY_LINEAR"
            negID = find(cell2mat(read_vars(i))<0);
            CyGNSS_product.negative_reflects_num =  string(size(negID, 1));
            Reflect = cell2mat(read_vars(i));
            Reflect(negID) = NaN;
            eval([newvar ' = accumarray([cygnss_r cygnss_c],Reflect,[],@computemean,-9999);']);
            vars_after_accum{i} = eval(newvar); %to svae the read variables in a structure
            [a,b] = size(vars_after_accum{i});
            correct_matrix(1:a, 1:b) = cell2mat(vars_after_accum(i));
            correct_matrix(correct_matrix==-9999)=NaN;
